--------------DAY 4 NT-------------------
--22-AGGREGATE Fonk.: toplamsal fonk : MAX,MIN, COUNT, SUM,AVG
...
--calisanlar3 tablosunda max maasi goruntule
SELECT MAX(maas) FROM calisanlar3;
--calisanlar3 tablosunda min maasi goruntule
SELECT MIN(maas) FROM calisanlar3;
--calisanlar3 tablosunda ortalama maasi goruntule
SELECT AVG(maas) FROM calisanlar3;
SELECT ROUND(AVG(maas),2) FROM calisanlar3; --ROUND() kac decimal sayi dondursun
--calisanlar3 tablosunda toplam maasi goruntule
SELECT SUM(maas) FROM calisanlar3;
--calisanlar3 tablosunda kayit sayisini goruntule
...
--23-ALIASES:rumuz/etiket/takma isim/nickname : tablo veya sutunlara gecici isim verebiliriz
...
SELECT calisan_isim  isim FROM workers AS w; -- alternativ kullanim AS kullanmadan


--24-SUBQUERY--NESTED QUERY
--24-a-SUBQUERY: WHERE ile kullanımı

--marka_id si 100 olan markada calisanlari listeleyiniz.
SELECT marka_isim FROM markalar WHERE marka_id=100;  --VAKKO MARKASI
...
--INTERVIEW QUESTION:calisanlar3 tablosunda ikinci en yuksek maaşı gosteriniz.ODEV
SELECT MAX(maas) AS ikinci_en_yuksek_maas
FROM calisanlar3
WHERE maas < (SELECT MAX(maas) FROM calisanlar3);

--calisanlar3 tablosunda max veya min maasi alan calisanlarin tum fields gosteriniz
SELECT * 
FROM calisanlar3
...
-- marka_id'si 101'den buyuk olan marka calisanlarin tum bilgileri listeleyiniz.
...
-- Her markanin ismini, calisan sayisini ve o markaya ait calisanlarin maksimum ve minimum maasini 

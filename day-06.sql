...
-- 29 - GROUP BY clause : tablomuzdaki kayitlari belirli bir veya daha fazla sutuna gore gruplandirabiliriz.
...
FROM manav
GROUP BY isim
ORDER BY SUM(urun_miktar) DESC;  -- sum yerine toplam_kg da kullanabiliriz
...
--NOT:GROUP BY ile gruplama yapıldığında SELECTden sonra sadece gruplanan sütun
...
-- urun ismine gore her bir urun alan toplam kisi sayisi gosteriniz.
...
SELECT isim, COUNT(isim)  musteri_sayisi
FROM manav
GROUP BY isim
...
--Eğer bir şehirde alınan MAX maas 5000’den düşükse sehir ismini ve MAX maasi veren sorgu yazınız.odev
